...
gulp.task('uglify', ['usemin'], function() {
  return gulp.src(runPath + 'dist/assets/scripts/*.js')
    .pipe($.plumber())
    .pipe(debug())
    .pipe($.uglify())    
    .pipe(gulp.dest(runPath + 'dist/assets/scripts/'));
});
...
gulp.task('build', ['sass:dist', 'usemin', 'images', 'fonts:dist', 'copy:vendorjs', 'uglify'], function(){
});